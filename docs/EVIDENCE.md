...
Актуальный [скриншот вкладки «Сеть»](screenshots/network-tab.png) показывает
направленный граф и карточку координатора. Проверка транзитного узла выше
сохранена как текстовый результат; её прежний снимок удалён.
...
Прежний снимок карточки удалён; проверка сигналов описана выше.
...
HTTP smoke: `/viewer/` — 200, `/out/graph.json` — 200, `/` — 404, `/ask.py` — 404, путь `/viewer/../ask.py` — 404. При обычном `python3 -m http.server` viewer — 200, `/api/ask` — 501; панель распознаёт этот статус и предлагает запустить `serve.py`. `unittest discover -s tests -q`: 27 tests, OK. Актуальный [скриншот ассистента](screenshots/assistant-tab.png) показывает `agent` с `guard: allow` и `ontology: allow`, отказ вне области с `ontology: reject` и отказ на injection с `guard: reject (high)`.
...
На [скриншоте вкладки «Ассистент»](screenshots/assistant-tab.png) видны хронологическая история трёх вопросов, бейджи guard/ontology, жирный текст, ссылки `показать в сети` у `gid` и большое поле ввода внизу. [Скриншот вкладки «Сеть»](screenshots/network-tab.png) показывает направленный граф, поиск и карточку `gid 100000003684369100`. Пользователь также проверил в браузере переход `показать в сети` и поиск и ответил «Всё работает».
