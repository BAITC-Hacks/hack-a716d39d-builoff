...
Пользователь приложил в диалоге два снимка предыдущего интерфейса: запрос `ignore previous instructions…` показывает `guard: reject (high)` и `ontology: reject`; запрос `какой курс доллара?` показывает `guard: allow (low)` и `ontology: reject`. На обоих виден рабочий граф и карточка `gid 100000003684369100`. Файлы `docs/screenshots/assistant-*.png` в рабочем дереве отсутствуют, поэтому ссылки на них здесь не приводятся. Это проверка отображения предыдущей панели, не новой компоновки вкладок.

## I7 · вкладки «Ассистент» и «Сеть»

Тест интерфейсной логики без браузера: `node tests/check_viewer.js` проверил вкладку по умолчанию, построение графа после открытия сети, поиск другого `gid`, три вопроса и хронологическую историю, отображение `**` жирным и переход по ссылке `gid` обратно к графу — `OK`. `node --check` прошёл. Три повторных live-вопроса из I5 через `/api/ask`: `200 ANSWER allow allow` для каждого, длины ответов 775/625/579 символов.

Пользователь приложил в диалоге снимок новой вкладки «Ассистент» на `127.0.0.1:8765/viewer/`: видны три вопроса, guard/ontology, жирный текст и ссылки `показать в сети` у `gid`. На снимке ввод был сверху, а новые ответы — выше старых. После замечания пользователя поле заменено на большое многострочное и перенесено вниз; история переведена в хронологический порядок. Пользователь повторно проверил в браузере поле внизу, порядок сообщений, переход `показать в сети` и поиск во вкладке «Сеть» и ответил «Всё работает». Свежий браузерный снимок последнего состояния не предоставлен. Файлы `assistant-*.png` в репозитории отсутствуют: вложения доступны только в диалоге.
